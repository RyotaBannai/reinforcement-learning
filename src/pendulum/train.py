...
class Agent:
    ...
    def update_batch(
        ...
        if gamma is None:
            gamma = self.gamma  # ← self.gammaと揃える

        S = torch.as_tensor(np.array(buf["s"]), dtype=torch.float32)
        S_next = torch.as_tensor(np.array(buf["s_next"]), dtype=torch.float32)
        U = torch.cat(buf["u"], dim=0).view(-1, 1)  # [T,1]
        OLD = torch.cat(buf["old_logp"], dim=0).to(dtype=torch.float32)  # [T,1]
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(len(S))):
                gae = delta[t] + gamma * lam * not_done[t] * gae
                adv[t] = gae
            target = adv + V
            # 標準化＋外れ値クリップ（安定化に効く）
        ...
        for _ in range(epochs):
            ...
            for i in range(0, T, minibatch):
                ...
                new_logp = (logp_u - corr).sum(dim=-1, keepdim=True)  # [mb,1]
                ratio = (new_logp - OLD[b].to(new_logp.device)).exp()  # [mb,1]
                ...
                # ent_coef = max(1e-4, 1e-3 * (0.995 ** (episode // 20)))
                # loss_pi = -torch.min(pg1, pg2).mean() - ent_coef * entropy
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if len(buffer["s"]) == T or done:
                agent.update_batch(buffer, episode, agent)
                buffer = dict(s=[], s_next=[], r=[], u=[], old_logp=[], term=[])
        if episode % 100 == 0:
            eval_rewards = []
